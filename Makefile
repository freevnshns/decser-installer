...
default:
	@echo "run \"make ot\" and after that run \"make full\""
...
full: pkg-deps user apache aria mediatomb supervisor owncloud system-manager web-application xmpp owncloud updater
...
updater:
	mkdir -p /updater/
	cp updater.sh /updater/updater.sh
